...
for (j in 1:20) {
  ...
  for (t in 77:82) { 
    phylopmulti[i, t] <- phylop[i,j] <- summary(phyloglm(extant ~ ., data = data.frame(temp), btol = 30, method="logistic_IG10", phy = tree[[t]]))$coefficients[2,"p.value"]
    #phylopmulti[t] <- rowMedians(phylop)
