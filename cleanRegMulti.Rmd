...
var = var[-c(1,2,3, 5),1] #remove chars and extant

for (i in var) { model = paste0("extant ~ mass.g +", i)
                tic("Time")
...
for (imps in 1:length(imp$imp[[1]])) {
    ...
    random_sample <- createDataPartition(temp.matrix$extant, p = 0.8, list = FALSE)
...
plot(weights123$`model$weights`[weights123$`model$weights` < 0.16])
text(weights123$`model$weights`[weights123$`model$weights` < 0.16], labels = rownames(weights123)[weights123$`model$weights` < 0.16], cex = 0.7)
