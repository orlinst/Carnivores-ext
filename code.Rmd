...
data_filter(speciesE, extant == 0 & is.na(diet.new))
...
data1 <- speciesE[c(cols)]
rownames(data1) <- speciesE$species_name

#convert vars to what they need to be - factor, numeric etc (if need be)
...
#p2 <- ggplot(data1, aes(x=extant, y=tree1$tip.label)) + 
...
p1 <- gheatmap(circ, data[cols], offset=.8, width=.2,colnames_angle=95, colnames_offset_y = .25)
