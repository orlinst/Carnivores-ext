...
prcomp.multi.impute <- function (imputedsets) {
  prset <- matrix(ncol=100, nrow=100)
  for (i in 1:imputedsets){
princomp(complete(imp, i)[, -c(3, 6, 10, 11)]) # only continuous vars
  prset <<- as.data.frame(capture.output(b2$loadings[,1]), )
}
    }
```
