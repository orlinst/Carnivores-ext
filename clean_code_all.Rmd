...
imp <- mice(data_final, 20, maxit = 100, method = c("", "", "logreg", "polyreg", "midastouch", "midastouch", "midastouch", "midastouch", "midastouch", "midastouch", "polyreg", "polyreg", "midastouch", "midastouch", "polyreg", "polyreg", "midastouch", "midastouch", "midastouch", "midastouch", "midastouch","midastouch", "logreg", "logreg","logreg","logreg","logreg", "midastouch"))
...
d2 = data.matrix(dataX1[,c(5:10,13,14,17:28)]) #select and break down categorical variables
...
for (j in 1:20) {                         #20 = number of datasets, 134 = numbher of species
    n = data.matrix(get(paste0("dataX", j))[,c(11)])
    loctype <- table(1:134, n)
    m1 = matrix(unlist(loctype), nrow(loctype), ncol(loctype), dimnames=list(1:134, colnames(loctype)))
    
    n = data.matrix(get(paste0("dataX", j))[,c(12)])
    diet.cat <- table(1:134, n)
    m2 = matrix(unlist(diet.cat), nrow(diet.cat), ncol(diet.cat), dimnames=list(1:134, colnames(diet.cat)))
    
    n = data.matrix(get(paste0("dataX", j))[,c(15)])
    activity <- table(1:134, n)
    m3 = matrix(unlist(activity), nrow(activity), ncol(activity), dimnames=list(1:134, colnames(activity)))
    
    n = data.matrix(get(paste0("dataX", j))[,c(16)])
    soc <- table(1:134, n)
    m4 = matrix(unlist(soc), nrow(soc), ncol(soc), dimnames=list(1:134, colnames(soc)))
    
    d[[j]] = data.matrix(get(paste0("dataX", j))[,c(5:10,13,14,17:28)])
  ...
  #for (i in 1:ncol(m)) {
    #add var numbers
  for (i in 1:41) {                     #42 = the number of variables after breaking down categorical vars
...
aaa <- cbind(rowMedians(p[1:41,]), colnames(m)) #non phylo output
