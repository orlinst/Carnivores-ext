...
#1. Load data and prep the dataset
```{r}
require(psych)
...
speciesE$simpson <- log(1/(speciesE$simpson+0.00000001))
...
#for (i in 1:length(colnames(speciesE))){
#    if (class(speciesE[[i]]) == "numeric") 
#        speciesE[[i]] <- #normalize(speciesE[[i]], na.rm = TRUE)}


data_final$activity <- michaud_additional$Activity.pattern[w]
data_final$soc.complexity <- michaud_additional$Social.complexity[w]
data_final$gr.size <- michaud_additional$Group.size[w]
data_final$ges.days <- michaud_additional$Gestation.days[w]
data_final$interbirth.days <- michaud_additional$Interbirth.days[w]
data_final$weaning.days <- michaud_additional$Weaning.days[w]


data_final$activity <- as.factor(data_final$activity)
data_final$soc.complexity <- as.factor(data_final$soc.complexity)

data_final$ges.days <- log(data_final$ges.days)
data_final$interbirth.days <- log(data_final$interbirth.days)
data_final$weaning.days <- log(data_final$weaning.days)
data_final$gr.size <- log(data_final$gr.size)

data_final$ges.days <- normalize(data_final$ges.days, na.rm = TRUE)
data_final$interbirth.days <- normalize(data_final$interbirth.days, na.rm = TRUE)
data_final$weaning.days <- normalize(data_final$weaning.days, na.rm = TRUE)
data_final$gr.size <- normalize(data_final$gr.size, na.rm = TRUE)
...
extant <- match(data_final$species, extant_large_mammals$V1)
extinct <- match(data_final$species, extinct_large_mammals$V1)
...
extinct123 <- which(!is.na(check[,2]))
data_final$species[extinct123]
data_final$extant <- 1
data_final$extant[extinct123] = 0
data_final[,c("extant", "species")]
data_final = data_final[,c(1:4, 21, 5:20)]
...
speciesE$longevity <- X_helbstab_journal_pone_0261185_s005$`Maximum longevity (years)`[m1]
...
p4 <- dotplot(factor(dataX1$extant) ~ (dataX1$litter))
p3 <- dotplot(factor(dataX17$extant) ~ (dataX17$litter))
p2 <- dotplot(factor(dataX11$extant) ~ (dataX11$litter))
p1 <- dotplot(factor(dataX9$extant) ~ (dataX9$litter))
...
results <- correlation(data_final)
summary(results)
...
#Adding the 5 missing species (length = 0.5 on the branch - except for Conepatus = 0 from the mrca)
...
#2. Load final tree/data and clean data
```{r}
require(phytools)
require(mulTree)
...
mi.glm(tree, 5, extant ~ longevity + mass.g + litter, 20, alpha=0.001)
...
h <- hist(pvals[,2], breaks=50, plot=FALSE)
cuts <- cut(h$breaks, c(-Inf, 0.05,1))
plot(h, col=c("firebrick", "white")[cuts], main=paste(colnames(pvals)[2], ": Histogram of p-values \n N =", nrow(pvals), "Trees = ", nrow(pvals)/25, "\n ", sum(pvals[,2] <= 0.05), "<=0.05", " \n median =", signif(median(pvals[,2]))), xlab="p-values", xaxt='n', cex.main=0.8)
text(h$mids,h$counts,labels=h$counts, adj=c(0.5, -0.5),)
axis(1, at = seq(0, 1, by = 0.01), las=2)
abline(v=median(pvals[,2]),col="blue",lwd=3)
arrows(x0 = 0.05,
...
h1 <- hist(estimates[,2], breaks = 25, main=paste("Histogram of estimates \n N=", nrow(estimates), "median =", signif(median(estimates[,2]))), xlab="Estimates", col="darkmagenta", cex.main=0.8)
text(h1$mids,h1$counts,labels=h1$counts, adj=c(0.5, -0.5))
...
h2 <- hist(pvals[,3], breaks=50, plot=FALSE)
cuts <- cut(h2$breaks, c(-Inf, 0.05, 1))
plot(h2, col=c("firebrick", "white")[cuts], main=paste(colnames(pvals)[3], ": Histogram of p-values \n N =", nrow(pvals), "Trees = ", nrow(pvals)/25,"\n ", sum(pvals[,3] <= 0.05), "<=0.05", " \n median =", signif(median(pvals[,3]))), xlab="p-values", xaxt='n', cex.main=0.8)
text(h2$mids,h2$counts,labels=h2$counts, adj=c(0.5, -0.5),)
axis(1, at = seq(0, 1, by = 0.01), las=2)
abline(v=median(pvals[,3]),col="blue",lwd=3)
arrows(x0 = 0.05,
       y0 = 0.001,
       x1 = 0.05,
       y1 = 100,
...
h3 <- hist(estimates[,3], breaks = 25, main=paste("Histogram of estimates \n N=", nrow(estimates), "median =", signif(median(estimates[,3]))), xlab="Estimates", col="darkmagenta", cex.main=0.8)
...
```{r}
for (t in 1:1000) {
    assign(paste0("model",t), with(imp, phyloglm(extant ~ mass.g, tree[[t]], method = c("logistic_MPLE"))))}

for (i in 1:20){
    temptree <- tree[[i]]
    as.mira(models) <- with(imp, phyloglm(extant ~ mass.g, phy=temptree, method = c("logistic_MPLE")))}



for (t in 1:3) {
   paste0("model",t) <- with(imp, {phyloglm(extant ~ mass.g, phy=tree[[t]], method = c("logistic_MPLE"))})}
...
for (t in 1:2) {
for (imps in 1:20) {
      model <- with(imp, phyloglm(extant ~ mass.g, data =get(paste0("dataX", imps)), phy=tree[[t]], method = c("logistic_MPLE")))}}
...
for (t in 1:1000){
model[t] <- with(imp, {
        phyloglm(extant ~ mass.g, phy=tree[[t]], method = c("logistic_MPLE"))
  })
}
                    
                    
        
x <- c(sample(1:1000, 1), sample(1:1000, 999, replace = T))
length(unique(x))
sample(x, 1000)
x
...
regress.multi.impute(tree, 20, extant ~ mass.g + home.range + eq + pc + litter + simpson + shannon + abundance + longevity, 1, 3)
#full model with simpason/shannon
#regress.multi.impute(tree, 10, extant ~ mass.g + home.range + brain + pc + BMR + litter + simpson + abundance + loctype + diet.cat, 1, 9)
#pc!
...
data.pc <- cbind(data.pc, diet.cat, loctype)
...
#data.pc$extant <- NULL
data.pc$Extant1 <- NULL
data.pc$Extinct0 <- NULL

#data.pc$extant <- as.numeric(data.pc$extant)

#Impute data.pc
require(mice)
#imp <- mice(data.pc, 20, maxit = 5, method = "norm")
...
for(i in 1: 20) {
    assign(paste0("dataX",i), data_final$species_name)
}
...
#add loadings
for (i in paste0("dataX", 1:length(imp$imp[[1]]))) {
    assign(i, cbind(get(i), MeanS))
}

#data$extant <- as.numeric(data$extant) (keep the broken down?)

for (i in paste0("dataX", 1:length(imp$imp[[1]]))) {

   assign(i, get(i)[-c(11:21)])                    #remove broken down cats (CHECK COLS!)
...
loctype <- as.data.frame(loctype)

for(imputedsets in 1: length(imp$imp[[1]])) {
    assign(paste0("dataX", imputedsets), cbind(get(paste0("dataX", imputedsets)), loctype))}
...
#Phylosig
require(sensiPhy)
asdasd <- miss.phylo.d(data_final, tree[[3]], binvar= home.range)
print(asdasd)
plot(asdasd)
...
vioplot(data_final$BMR ~ data_final$IUCN.status)
vioplot(log(carn.ext$BMR)[carn.ext$IUCN.status == "Least Concern"])
(anova)
...
data2 <- data1[-c(1,3,4)]   #only status
...
ggplot(dataX1) + 
    geom_density_ridges(aes(x = BMR, y = activity, group = interaction(activity, extant),fill = extant), alpha = 0.7)  + 
...
ggplot(dataX15) + 
    geom_density_ridges(aes(x = dataX15$BMR, y = Activity, group = interaction(Activity, Status),fill = Status), alpha = 0.7) +
          theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title=element_text(size=12,face="bold")) +  scale_x_continuous(name="Ln Basal metabolic rate (ml O2/h)") + scale_fill_manual(values=c("firebrick", "steelblue"))
...
p <-ggplot(output, aes(log2(No)+10, reorder(Variable, No))) +
  ...
  geom_vline(xintercept=5.68, linetype = "dotted") +
  geom_vline(xintercept=2.36, linetype = "dotted") +
  annotate("text", x=5.58, y=35, label="p<0.05", angle=90, size = 3.3, colour="dodgerblue3") +
  annotate("text", x=2.26, y=35.2, label="p<0.005", angle=90, size = 3.3, colour="dodgerblue3") +
  scale_x_continuous(breaks=c(1.03, 3.35, 4.94, 5.67, 8), labels=c("1.03" = "0.002", "3.35" ="0.01", "4.94" = "0.03",
                              "5.68" = ">0.05", "8" = ""))
  #scale_x_break(c(1.8, 3), scales=2)
    #geom_text(data = right_label, aes(color = Run1, label = round(No, 2)),
                  #size = 3, hjust = -.5, position = position_dodge(0.5)) +
    #geom_text(data = left_label, aes(color = Run1, label = round(No, 2)),
                  #size = 3, hjust = 1.5, position = position_dodge(0.5))
...
p21 <-  gheatmap (p11, dataX1[,c("activity", "mass.g", "BMRres")], offset=-0.5, width=0.5, low=(values=pal[1]), high=(values=pal[2]), colnames_position = "top", font.size=4) + scale_fill_manual(values = pal)
...
dataX1 %>%
    mutate(extant = factor(extant)) %>%
    ggplot() +
    geom_boxplot(aes(x = extant, y = mass.g)) + theme(panel.grid.major = 
    element_blank(), panel.grid.minor = element_blank(), panel.background =
    element_blank(), axis.line = element_line(colour = "black"))

ggplot(dataX1) + 
    geom_density_ridges(aes(x = mass.g, y = activity, group = interaction(activity, extant),fill = extant), alpha = 0.7)  + theme(panel.grid.major = 
    element_blank(), panel.grid.minor = element_blank(), panel.background =
    element_blank(), axis.line = element_line(colour = "black"))
...
dev.new()
plot(PCAscores[,1:10],  # x and y data
     pch=21,           # point shape
     col=PCAcolors,    # point border color
     bg=PCAcolors,     # point color
     cex=1.5,          # point size
...
models <- define_model_set(
    one   = c(at ~ mass.g, home.range ~ mass.g, extant~at, extant~home.range),
    two = c(extant ~ at + home.range + mass.g),
    three   = c(extant ~ mass.g, at ~ extant, home.range ~ extant)
)
    
plot_model_set(models)


result <- phylo_path(models, data = dataX1, tree = tree[[1]], model = 'lambda', method = "logistic_MPLE")
s <- summary(result)
plot(s)
...
#dataX2$extant <- as.numeric(dataX1$extant)

pathmodelLV <-'#path analysis
#measurement model
somatic=~ BMR + mass.g + longevity
locomotion=~ at + pc + home.range
#regression model
extant ~ locomotion + somatic
litter ~ locomotion + somatic
'
fit1 <- sem(pathmodelLV, data = dataX1)
semPaths(fit1,   title = FALSE,
         curvePivot = TRUE,
         what = "std",
         rotation = 2,
         layout = "tree2",
...
pathmodelLV <-'#path analysis
#measurement model
somatic=~ BMR + mass.g + brain + home.range
locomotion =~ at + pc + abundance
#regression model
extant ~ locomotion + somatic
litter ~ locomotion + somatic
'

fit1 <- sem.mi(pathmodelLV, data = imp)


model_fit <- sem(pathmodelLV,  data=complete(imp,1))
SEMPLOT <- semPlot::semPlotModel(model_fit)
desired_output <- data.frame(standardizedsolution(fit1))
SEMPLOT@Pars$std <- desired_output$est.std
semPaths(SEMPLOT,  title = FALSE,
         curvePivot = TRUE,
         what = "std",
         rotation = 2,
         layout = "tree2",
