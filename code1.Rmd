...
colnames(extant) <- c("extinct", "extant") #so they can be easily distinguished
...
for (component in 1 : length(PCA1$loadings[,1])) {     #number of components
  ...
  assign(paste("scores",component, "-", imputedsets, sep=""), (get(paste("PCA",imputedsets, sep=""))$scores)[,component]) 

  #put all loadings and scores PER set in separate objects
  #this works
  
  assign(paste0("PC",component, sep=""), data.frame(do.call(cbind, mget(ls(pattern = paste0("\\bloadings", component, "\\b"))))))
  assign(paste0("Score",component, sep=""), data.frame(do.call(cbind, mget(ls(pattern = paste0("\\bscores", component, "\\b"))))))
...
for (component in 1 : length(PCA1$loadings[,1])) {
  ...
  assign(paste0("PC",component, sep=""), get(paste0("PC",component, sep=""))[-1])
...
for (i in paste0("PC", 1:length(PCA1$loadings[,1]))){
  d=get(i)
  colnames(d)[1]='mean'
  assign(i,d)
}
...
for (i in paste0("Score", 1:length(PCA1$loadings[,1]))) {
    m = get(i)[1]
    MeanLoadings[i] <- cbind(m)
    #assign(i, m)
}

MeanLoadings[1:6] <- NULL
